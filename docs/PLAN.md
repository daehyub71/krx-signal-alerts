...
```
              ...
              └────────┬──────┘     │
                       ...
                       │      └─────┬─────────┘
                       └────────────┤
                                    │  fan-out ─ 2개 병렬 (F13c 오류 격리)
                             ┌──────┴──────┐
                             ▼             ▼
                       send_kakao      send_email
                             └──────┬──────┘
                             ┌──────▼──────┐
...
| 발송 2개 fan-out | **두 노드 모두 예외를 밖으로 내지 않는다.** 각자 `SendResult`를 상태에 적고, 실패 판정은 `finalize` 한 곳에서 한다 (F13c) |
...
**노드별 재시도**: `load_bars` · `send_kakao` · `send_email`에만 LangGraph `RetryPolicy`
(최대 3회, 지수 백오프)를 건다. 순수 계산 노드는 재시도해도 같은 결과라 걸지 않는다.
